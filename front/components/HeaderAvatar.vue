<template>
  <v-menu transition="slide-y-transition" min-width="200px" rounded offset-y>
    <template v-slot:activator="{ on }">
      <v-btn depressed x-large v-on="on" tile>
        <template v-if="currentUser.image.url !== defaultImage">
          <v-avatar size="44">
            <v-img :src="currentUser.image.url" alt="avatar" />
          </v-avatar>
        </template>
        <template v-else>
    ...
    <v-card>
      <v-list-item-content class="justify-center">
        <div class="mx-auto text-center">
          ...
          <v-btn depressed rounded text @click="pagelink(links[0].to)">
            マイページ
          </v-btn>
          <v-divider class="my-3"></v-divider>
          <v-btn depressed rounded text @click="pagelink(links[1].to)">
            登録情報変更
          </v-btn>
          <v-divider class="my-3"></v-divider>
...
export default {
  data() {
    return {
      ...
      links: [{ to: "/users/_id" }, { to: "/users/edit" }],
    };
  },
  computed: {
    ...mapGetters({
